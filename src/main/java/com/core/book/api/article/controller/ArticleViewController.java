...
public class ArticleViewController {
    ...
    @Operation(
            ...
            description = "게시글 목록을 조회합니다. / last : true일 경우 마지막 데이터라는 의미입니다. / 페이지는 0부터 시작합니다. / 게시글전체 조회 타입 : all, 감상평 조회 타입 : review, 인상깊은구절 조회 타입 : phrase, QnA 조회 타입 : qna, 인용 조회 타입 : quotation, 추천해주세요 조회 타입 : recommend"
    )
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponse<ReviewArticleDetailDTO>> getReviewArticleDetail(
    ...
    public ResponseEntity<ApiResponse<PhraseArticleDetailDTO>> getPhraseArticleDetail(
            @PathVariable Long id,
            @AuthenticationPrincipal UserDetails userDetails
    ) {
